...
def afterlogin_view(request):
    if is_student(request.user):      
        ...
        account = SMODEL.Student.objects.all().filter(user_id=request.user.id,status=True)
        if account:
            return redirect('student/student-dashboard')
        else:
...
def take_exam_wait_view(request):
    if is_student(request.user):
        ...
        if account:
            ...
            return render(request, 'exam/student_wait_for_exam.html')
...
def teacher_view_pending_student_view(request):
    students= SMODEL.Student.objects.all().filter(status=False)
    return render(request,'exam/teacher_view_pending_exam.html',{'students':students})

@login_required(login_url='teacherlogin')
def approve_student_for_exam_view(request,pk):
    student=SMODEL.Student.objects.get(id=pk)
    user=User.objects.get(id=student.user_id)
    user.save()
    student.status = True
    student.save()
    return HttpResponseRedirect('/teacher-view-pending-exam')


@login_required(login_url='teacherlogin')
def reject_student_for_exam_view(request,pk):
    student=SMODEL.Student.objects.get(id=pk)
    user=User.objects.get(id=student.user_id)
    user.delete()
    student.delete()
...
def teacher_student_view(request):
    ...
    return render(request,'exam/teacher_student.html',context=dict)
...
def teacher_view_student_marks_view(request):
    students= SMODEL.Student.objects.all()
    return render(request,'exam/teacher_view_student_marks.html',{'students':students})

@login_required(login_url='teacherlogin')
def teacher_view_marks_view(request,pk):
    courses = models.Course.objects.all()
    response =  render(request,'exam/teacher_view_marks.html',{'courses':courses})
...
def teacher_check_marks_view(request,pk):
    ...
    return render(request,'exam/teacher_check_marks.html',{'results':results})
...
@login_required(login_url='adminlogin')
def admin_student_exams_pending_view(request):
    pending_exams= QMODEL.StudentExam.objects.all().filter(status=False)
    return render(request,'exam/admin_view_pending_student_exam.html',{'pending_exams':pending_exams})


@login_required(login_url='adminlogin')
def approve_student_exam_view(request,pk):
    student_exam=QMODEL.StudentExam.objects.get(id=pk)
    student_exam.status = True
    student_exam.save()
    return HttpResponseRedirect('/admin-student-exams-pending')



@login_required(login_url='adminlogin')
def reject_student_exam_view(request,pk):
    student_exam=QMODEL.StudentExam.objects.get(id=pk)
    student_exam.delete()
    return HttpResponseRedirect('/admin-student-exams-pending')